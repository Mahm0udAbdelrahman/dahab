...
<form action="{{ route('home') }}#products-section" method="GET" class="search-form d-flex align-items-center gap-2">
    ...
    <button class="btn btn-danger search-btn" type="submit">{{ __('Search') }}</button>
                ...
                <!-- ACCOUNT / LANGUAGE / WISHLIST / CART -->
                <div class="col-md-3">
                    <div class="header-ctn d-flex justify-content-end align-items-center gap-4">
                        ...
                        <!-- Wishlist -->
                        <div>
                            <a href="{{ route('favorites.index') }}" class="d-flex align-items-center gap-1 text-decoration-none text-dark">
                                <i class="fa fa-heart-o fa-lg"></i>
                                <span>{{ __('Wishlist') }}</span>
                                <div class="qty badge bg-danger rounded-circle text-white">{{ $favoriteCount }}</div>
                            </a>
                        </div>

                        <!-- Cart -->
                        <div class="dropdown">
                            <a class="dropdown-toggle d-flex align-items-center gap-1 position-relative text-decoration-none text-dark" data-toggle="dropdown" href="#">
                                <i class="fa fa-shopping-cart fa-lg"></i>
                                <span>{{ __('Cart') }}</span>
                                <div class="qty badge bg-danger rounded-circle text-white position-absolute top-0 start-100 translate-middle">{{ $cartCount }}</div>
                            </a>

                            <div class="cart-dropdown dropdown-menu dropdown-menu-right p-3 shadow rounded" style="min-width: 320px; max-height: 350px; overflow-y: auto;">
                                <h5 class="mb-3">{{ __('Cart Items') }} ({{ $cartCount }})</h5>

                                <ul class="cart-items-list list-unstyled mb-3">
                                    @forelse ($cartItems as $cart)
                                        <li class="mb-3 d-flex align-items-center gap-3">
                                            <img
                                                src="{{ asset($cart->product->images->first()->image ?? 'default.png') }}"
                                                alt="product"
                                            ...
                                            <form action="{{ route('carts.destroy', $cart->id) }}" method="POST" class="mb-0">
                                                @csrf
                                                @method('DELETE')
                                                <button class="delete btn btn-link p-0 text-danger" aria-label="Remove item">
                                ...
                                @if ($cartCount > 0)
                                    <div class="cart-summary d-flex justify-content-between fw-bold border-top pt-2">
                                        <small>{{ $cartCount }} {{ __('items') }}</small>
                                        <span>${{ number_format($cartItems->sum(fn($c) => $c->product->price), 2) }}</span>
                                    </div>

                                    <div class="cart-btns mt-3 d-flex justify-content-between">
                                        <a href="{{ route('carts.index') }}" class="btn btn-primary btn-sm">
                                            {{ __('View Cart') }}
                                        </a>
                                    </div>
                                @endif
